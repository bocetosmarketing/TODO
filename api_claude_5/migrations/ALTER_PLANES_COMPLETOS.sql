...
ADD COLUMN IF NOT EXISTS `price` DECIMAL(10,2) NOT NULL DEFAULT 0 AFTER `name`,
ADD COLUMN IF NOT EXISTS `currency` VARCHAR(3) DEFAULT 'EUR' AFTER `price`,

-- LIMITS
ADD COLUMN IF NOT EXISTS `requests_per_day` INT DEFAULT -1 COMMENT '-1 = ilimitado' AFTER `tokens_per_month`,
ADD COLUMN IF NOT EXISTS `requests_per_month` INT DEFAULT -1 COMMENT '-1 = ilimitado' AFTER `requests_per_day`,
ADD COLUMN IF NOT EXISTS `max_words_per_request` INT UNSIGNED DEFAULT 2000 AFTER `requests_per_month`,
ADD COLUMN IF NOT EXISTS `max_campaigns` INT DEFAULT -1 COMMENT '-1 = ilimitado' AFTER `max_words_per_request`,

-- TIMING (CRÍTICO)
ADD COLUMN IF NOT EXISTS `post_generation_delay` INT UNSIGNED DEFAULT 60 COMMENT 'Segundos de delay entre posts al ejecutar' AFTER `max_campaigns`,
ADD COLUMN IF NOT EXISTS `api_timeout` INT UNSIGNED DEFAULT 120 COMMENT 'Timeout en segundos para requests OpenAI' AFTER `post_generation_delay`,
ADD COLUMN IF NOT EXISTS `max_retries` TINYINT UNSIGNED DEFAULT 3 COMMENT 'Reintentos en caso de fallo' AFTER `api_timeout`,

-- FEATURES (JSON)
ADD COLUMN IF NOT EXISTS `features` JSON DEFAULT NULL COMMENT 'Features habilitadas del plan' AFTER `max_retries`;

-- Paso 2: Actualizar datos de planes existentes con valores V3
...
-- Paso 3: Insertar plan 'free' si no existe
