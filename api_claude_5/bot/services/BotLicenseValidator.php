...
class BotLicenseValidator {
    ...
    private function captureDomain($licenseId, $domain) {
        try {
            ...
            // Log de captura
            error_log("Chatbot API: Domain captured automatically - License ID: {$licenseId}, Domain: {$domain}");

        } catch (Exception $e) {
            error_log("Chatbot API: Error capturing domain - " . $e->getMessage());
        }
    }
